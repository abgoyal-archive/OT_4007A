...
static int tpd_probe(struct platform_device *pdev) {
  ...
  for(i = 1; i < TP_DRV_MAX_COUNT; i++)
  {
	tpd_load_mode=0;// add for normal mode
	/* add tpd driver into list */
	if(tpd_driver_list[i].tpd_device_name != NULL)
	{
		...
		if(tpd_load_status ==1) {
			TPD_DMESG("[mtk-tpd] normal model tpd_probe, tpd_driver_name=%s\n", tpd_driver_list[i].tpd_device_name);
			g_tpd_drv = &tpd_driver_list[i];
			break;
		}
	}
  ...
  if(tpd_load_status==0) //normal mode load failed!----------------------------llf
  {	
	  TPD_DMESG("[mtk-tpd][LLF] enter the repair mode \n");
	  tpd_load_mode=1; //enter repair mode
	  for(i = 1; i < TP_DRV_MAX_COUNT; i++)
	  {
	    		/* add tpd driver into list */
			if(tpd_driver_list[i].tpd_device_name != NULL)
			{
				...
				//msleep(1);
				if(tpd_load_status ==1) {
					TPD_DMESG("[mtk-tpd] repair mode tpd_probe, tpd_driver_name=%s\n", tpd_driver_list[i].tpd_device_name);
					g_tpd_drv = &tpd_driver_list[i];
					break;
				}
			}
	 ...
	 if(g_tpd_drv == NULL) {
	 	g_tpd_drv = &tpd_driver_list[1];//fix minitest fail by wwl
	 	if(tpd_driver_list[0].tpd_device_name != NULL) {
	  	...
	  	//return 0;//fix minitest fail by wwl
	  	} 
	  }	
    #ifdef CONFIG_HAS_EARLYSUSPEND
    MTK_TS_early_suspend_handler.suspend = g_tpd_drv->suspend;
