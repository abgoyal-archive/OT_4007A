...
static int tmd2771_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali, int cali_num);
/*----------------------------------------------------------------------------*/
typedef enum {
    CMC_BIT_ALS    = 1,
    CMC_BIT_PS     = 2,
...
static int tmd2771_init_client_for_cali(struct i2c_client *client)
{
   ...
   //---------read id:
	...
	databuf[1] = TMD2771_CMM_CONTROL_VALUE;//;0x20;//0xf0;//modify to 100mA, PR356949
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
	...
	databuf[0] = TMD2771_CMM_CONTROL;    
	databuf[1] = TMD2771_CMM_CONTROL_VALUE;//;0x20;//0xE0;//modify to 100mA, PR356949
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
...
static void tmd2771_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		//APS_LOG("tmd2771_eint_work close  = %d,far_away = %d,valid = %d",ps_cali.close,ps_cali.far_away,ps_cali.valid);
...
static int tmd2771_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali, int cali_num)
{
	...
	mdelay(10);		
	if(err = tmd2771_read_ps(client,tmp))
	{
	...
	while(tmp >= 1023)//判断Prox是否饱和
	{
	 ...
	 for(i = 0;i<M;i++)
	 {
		if(err = tmd2771_read_ps(client,&buffer[i]))
		{
				APS_ERR("tmd2771_read_data_for_cali fail: %d\n", i); 
				break;
		}
...
GET_THRE:
			...
			ps_data_cali->valid = 1;
			APS_LOG("tmd2771_read_data_for_cali close  = %d,far_away = %d,valid = %d, ofs = %d",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid, ps_data_cali->offset);
...
static int tmd2771_read_data_for_cali_for_call(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali, int cali_num)
{
     	int i=0 ,err = 0,j = 0;
	u16 buffer[21];
	u16 tmp = 0;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				...
				tmd2771_read_data_for_cali_for_call(obj->client,&ps_cali, 4);  //Note: We do not use first ps read data
				//APS_LOG("222*\n");
#endif
...
static int tmd2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err=tmd2771_read_data_for_cali(client,&boot_ps_cali, M))
		{
			boot_ps_ok=0;
		}
