...
static kal_uint16 OV8825Reg2Gain(const kal_uint16 iReg)
{
	...
	OV8825DB("[OV8825Reg2Gain]real gain= %d\n",(iGain +  (iGain * (iReg & 0x0F)) / 16));
...
static kal_uint16 OV8825Gain2Reg(const kal_uint16 Gain)
{
	...
	OV8825DB("[OV8825Gain2Reg]: isp gain:%d,sensor gain:0x%x\n",iGain,iReg);
...
{	
	...
	OV8825_write_cmos_sensor(0x3106,0x11);//15		//SRB_CTRL               
...
{	
	...
	//13fps OK  0x4837, 0x0D
	/*
	OV8825_write_cmos_sensor(0x3003, 0xce);////PLL_CTRL0
	OV8825_write_cmos_sensor(0x3004, 0xe0);//0xe0);//0xd8);////PLL_CTRL1, 
	OV8825_write_cmos_sensor(0x3005, 0x00);////PLL_CTRL2
	OV8825_write_cmos_sensor(0x3006, 0x20);//0x20);//0x10);////PLL_CTRL3
	OV8825_write_cmos_sensor(0x3007, 0x2b);//0x3b);////PLL_CTRL4
	*/
	//12fps  OK 0x4837, 0x28 ?
	//OV8825_write_cmos_sensor(0x3003, 0xce);////PLL_CTRL0
	//OV8825_write_cmos_sensor(0x3004, 0xe3);//0xe0);//0xd8);////PLL_CTRL1, 
	//OV8825_write_cmos_sensor(0x3005, 0x00);////PLL_CTRL2
	//OV8825_write_cmos_sensor(0x3006, 0x20);//0x20);//0x10);////PLL_CTRL3
	//OV8825_write_cmos_sensor(0x3007, 0x3b);//0x3b);////PLL_CTRL4

	//OV8825_write_cmos_sensor(0x3004, 0xd8);//0xe0);//0xd8);////PLL_CTRL1, 
	//OV8825_write_cmos_sensor(0x3005, 0x10);////PLL_CTRL2
	//OV8825_write_cmos_sensor(0x3006, 0x40);//0x20);//0x10);////PLL_CTRL3
	//OV8825_write_cmos_sensor(0x3007, 0x3b);//0x3b);////PLL_CTRL4
...
UINT32 OV8825Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		{
		    OV8825DB("OOV8825 READ ID error :%x",sensor_id);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	OV8825_write_shutter(ov8825.pvShutter); //modify by zhaojr
...
UINT32 OV8825GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    pSensorResolution->SensorPreviewHeight	= OV8825_IMAGE_SENSOR_PV_HEIGHT;
	#endif
    pSensorResolution->SensorFullWidth		= OV8825_IMAGE_SENSOR_FULL_WIDTH; //OV8825MIPI_REAL_CAP_WIDTH;
    pSensorResolution->SensorFullHeight		= OV8825_IMAGE_SENSOR_FULL_HEIGHT; //OV8825MIPI_REAL_CAP_HEIGHT;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    //pSensorInfo->SensorFullResolutionX= OV8825_IMAGE_SENSOR_PV_WIDTH;
    //pSensorInfo->SensorFullResolutionY= OV8825_IMAGE_SENSOR_PV_HEIGHT;
