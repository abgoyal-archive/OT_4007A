...
unsigned char ispaon=0;
//struct mutex ispaon_mutex;
static DEFINE_MUTEX(ispaon_mutex);
...
{
	printk("[Speaker_llf] The ispaon is ==%d \n",ispaon);	

	mutex_lock(&ispaon_mutex);
	if(ispaon==0){
		PRINTK("Sound_Speaker_Turnon channel = %d\n",channel);
		if(gsk_on){
			mutex_unlock(&ispaon_mutex);
			return;
		}
