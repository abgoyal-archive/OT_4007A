...
static unsigned short yamaha530_force[] = {0, YAS_MAG_I2C_SLAVEADDR, I2C_CLIENT_END, I2C_CLIENT_END};
...
/*--------------------yamaha power control function----------------------------------*/
static void yamaha530_power(struct mag_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "yamaha530"))
			{
				printk("power on fails!!\n");
			}
			else
				printk("power on yamaha530");
		}
		else	// power off
		{
			if (!hwPowerDown(hw->power_id, "yamaha530"))
			{
				printk("power off fail!!\n");
			}	
			else
				printk("power 0ff yamaha530");
		}
	}
	power_on = on;    
}
