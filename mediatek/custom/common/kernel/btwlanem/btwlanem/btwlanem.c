...
#include "../../drivers/hwver/inc/hw_check.h"
...
#define BT_IOCTL_GET_VER             _IOWR(BTWLAN_EM_IOC_MAGIC, 3, uint32_t)
...
static int btwlan_em_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case BT_IOCTL_GET_VER:
        {
            unsigned int ver = 0;
            BTWLAN_EM_DEBUG("BT_IOCTL_GET_VER\n");   
            mutex_lock(&pbtwlan_em->sem);
            ver = platform_is_3g_disable();
#ifdef	MTK_CNN_CBN
            ver = 1;
#endif
            BTWLAN_EM_DEBUG("BT_IOCTL_GET_VER=[%d]\n", ver); 
               
            mutex_unlock(&pbtwlan_em->sem); 
            if(copy_to_user((int __user *)arg, &ver, sizeof(unsigned int))){
        	return -EFAULT;
            }
            break;
        }
