...
void fan5405_set_ac_current(void)
{
    ...
    //1). 0x06h->0x10h -> 0x70 // set safety register first,
    //2). 0x01h->0xF8h -> 0xC8	//No Iinlim limit, Vlowv: 3.4V
    //3). 0x02h->0x8Eh -> Voreg = 4.2V, otg pin active high
    //4). 0x04h->0x19h -> 0x79 //Iterm = 97mA, Iocharge = 1250mA
    //5). 0x05h->0x04h		//Vsp = 4.533V
    fan5405_config_interface_liao(0x06,0x7a); //set ISAFE :1250mA, 4.2V
    fan5405_config_interface_liao(0x01,0xC8);
    fan5405_config_interface_liao(0x02,0x8E);
    fan5405_config_interface_liao(0x04,0x79); //set IOCHARGE
    fan5405_config_interface_liao(0x05,0x04);
    #endif
}
...
{ 
	...
	else
	{
		...
		if (Enable_BATDRV_LOG == 1) {
			printf("[BATTERY] bat_temperature_volt ==%d,BMT_status.temperature=%d\n",bat_temperature_volt,BMT_status.temperature);
		}
	}	
...
{    
    ...
    #endif	
    printf("[sunjiaojiao]mt6575_bat_fan5405");
    check_battery_exist();
