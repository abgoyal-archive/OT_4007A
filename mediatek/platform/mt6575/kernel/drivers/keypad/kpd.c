...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	if(kpd_enable_lprst && get_boot_mode() == NORMAL_BOOT) {
		...
		upmu_testmode_pwrkey_rst_en(0x01);
                ...
                //Susheng.ye.hz add for CR372702 begin
               #ifdef TARGET_BUILD_MMITEST
		upmu_testmode_homekey_rst_en(0x01);
               #else
		upmu_testmode_homekey_rst_en(0x0);//songlinhua mdf 20121123, enable force poweroff PMU without homekey long press
               #endif
               //Susheng.ye.hz add for CR372702 end
