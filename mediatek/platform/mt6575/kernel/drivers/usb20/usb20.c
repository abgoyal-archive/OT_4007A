...
{ .hw_ep_num =  1, .style = FIFO_RX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  2, .style = FIFO_TX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  2, .style = FIFO_RX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  3, .style = FIFO_TX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  3, .style = FIFO_RX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  4, .style = FIFO_TX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
{ .hw_ep_num =  4, .style = FIFO_RX,   .maxpacket = 512, .ep_mode = EP_BULK,.mode = BUF_SINGLE},
...
{
    DBG(3,"mt65xx_usb20_vbus++,is_on=%d\r\n",is_on);
    if(is_on){
...
bool musb_is_host(void)
{
	...
	mtk_set_vbus(mtk_musb,FALSE);
//	mt65xx_eint_mask(EINT_CHR_DET_NUM);
	msleep(delay_time);
	devctl = musb_readb(mtk_musb->mregs,MUSB_DEVCTL);
	DBG(2,"devctl = %x before set session\n",devctl);
	...
	{      
	...
	}else
		{
			...
			// disable_pll(MT65XX_UPLL,"USB_PLL");
			    ...
			    printk("disable VUSB and UPLL if usb not connect\n");
...
void musb_platform_enable(struct musb *musb)
{
  ...
  printk("enable VUSB and UPLL before connect\n");
	mdelay(10);     
	usb_phy_recover();
	mtk_usb_power = TRUE;
	musb->power = TRUE;
...
void musb_platform_disable(struct musb *musb)
{
   if(musb->power == FALSE)
	...
	hwPowerDown(MT65XX_POWER_LDO_VUSB,"VUSB_LDO");
	disable_pll(MT65XX_UPLL,"USB_PLL");
	printk("disable VUSB and UPLL before disconnect\n");
...
__tcmfunc irqreturn_t mt6575_usb_interrupt(int irq, void *dev_id)
{
...
int __init musb_platform_init(struct musb *musb)
{
	...
	device_create_file(musb->controller,&dev_attr_cmode);
	return 0;
}
