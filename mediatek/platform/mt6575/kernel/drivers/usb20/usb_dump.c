 ...
 */ 
volatile bool com_opend = false;  // Whether USB com port is opend
volatile bool usb_connected = true; // Whether USB is connected
...
{
	void __iomem *fifo = MUSB_FIFO_OFFSET(ep_num);
	int i = 0;
...
/**********************************************************
send data function, will send all data in the buffer,return send data count
**********************************************************/
...
/********************************************************************************
send/receive data function, will send/receive all data in the buffer,return send/receive data count
*********************************************************************************/
 ...
 // 	usb_connected = true;
    ...
    if(usb_connected)
    {
		if(com_opend)
	   	...
	   	}else if(!gadget_is_ready)
	   	{ // if gadget driver is not ready, we will enumration and wait com port open 
			data_ep_num = DATA_EP;
	   	 	config_usb_acm();
			if(com_opend)
			{
