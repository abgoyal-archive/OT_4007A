 ...
 *   and the Anroid Battery service for updating the battery status, ,charge with fan5405
 *
 * Author:
 * -------
 * James Lo
...
int Enable_BATDRV_LOG = 1;             //1:enable   0:disable  battery log
...
#define CHARGE_WITH_FAN5405	1         //1:enable 0:disable  charge with fan5405
	...
	if(work_mode == FAN5405_CHARGE_MODE)
	{
		switch(fault_bit)
		{
			...
			case CHARGE_TIMER_FAULT:
				/*re-start*/
				fan_struct->ops->set_con0_val(fan_struct, 0x01, 0x01);
...
//	        if(Enable_BATDRV_LOG >= 1)
		printk(KERN_INFO "jrd %s: CHR_ERROR_Now call pchr_turn_off_charging!\n",__func__);
	...
	else
	{
		...
		{
			printk(KERN_INFO "jrd %s: g_temp_CC_value:%d call pchr_turn_off_charging!\n",__func__,g_temp_CC_value);
			pchr_turn_off_charging();
		...
		{	
			...
			if(fan_struct)
			{
			        ...
			        fan_struct->ops->set_con4_val(fan_struct, 0, Iocharge, 1);      /*Iocharge = 550mA, Iterm = 100mA*/
...
{ 
	...
	else
	{
		...
		xlog_printk(ANDROID_LOG_WARN, "Power/Battery", "[BATTERY] bat_temperature_volt ==%d,BMT_status.temperature=%d\n",bat_temperature_volt,BMT_status.temperature);
	}
...
int BAT_CheckBatteryStatus(void)
{
	...
	if (Enable_BATDRV_LOG >= 1)
	{
	    	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:AVG(%d)] BatTemp:%d Vbat:%ld VBatSen:%ld SOC:%ld ChrDet:%d Vchrin:%ld Icharging:%ld ChrType:%d USBstate:%d gADC_I_SENSE_offset:%d\r\n", 
	       	CHARGING_FULL_CURRENT,BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
...
#else	/*move these codes to the end of this function*/
    ...
    if(battery_cmd_thermal_test_mode == 1)
   {
    ...
    else
    {
		...
		if(fan_struct && (fan_struct->work_mode == FAN5405_CHARGE_MODE))
		{
			...
			else if(((fan5405_status & 0x30) == 0x30) && (fan_struct->config_count == 0))	/*fault when charge config finish*/
			{
				check_fan5405_fault_status((fan5405_status & 0x07), 0);/*check fan5405 IC status when charging*/	
			}
		...
		{
			if ( (BMT_status.bat_charging_state == CHR_TOP_OFF) &&
	             (BMT_status.SOC == 100) &&  (BMT_status.bat_vol >= Batt_VoltToPercent_Table[10].BattVolt) )
			{
				if (Enable_BATDRV_LOG == 1)
				{
...
{
    int ret;
    xlog_printk(ANDROID_LOG_INFO, "[sunjiaojiao]", "mt6575_battery_fan5405");
