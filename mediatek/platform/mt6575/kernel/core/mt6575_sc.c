 ...
 */
extern int is_led_blinking(void);
...
wake_reason_t sc_go_to_sleep(unsigned int pwrlevel)
{
	...
	if (is_led_blinking()) {
		...
		mt_set_gpio_mode(110, 0);
		mt_set_gpio_dir(110, 1);
		mt_set_gpio_out(110, 1);
	}else {
		printk(KERN_INFO "led is not blinking\n");
		mt_set_gpio_mode(110, 0);
		mt_set_gpio_dir(110, 1);
...
MODULE_DESCRIPTION("MT6575 SLPCTRL Driver v4.2");
