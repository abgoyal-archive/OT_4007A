...
static stZSDPreviewSetting g_stMT6575ZSDPreview =
                    {
                        .SpinLock   = __SPIN_LOCK_UNLOCKED(g_stMT6575ZSDPreview.SpinLock),
                        .u4Dirty    =   0,
                    };
...
static inline int DescQueueRefill(unsigned long index)
{
    ...
    #define _CHECK_Q_BUSY_AND_RETURN_( _seg_name_ ) \
        if( _DescWriteWaitBusy( RGB_ROT0_BASE ) < 0 ){\
            ...
            return -1; }
    stZSDPreviewReg * pstReg = &g_stMT6575ZSDPreview.stReg;

    /*Fill descriptor command*/
    /*[SEG1]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG1]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg1[index] , MT6575_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));
...
static int MT6575_SetZSDPreview(stZSDPreviewReg * a_u4Param)
{
    unsigned long   flag;

    spin_lock_irqsave(&g_stMT6575ZSDPreview.SpinLock , flag);
    g_stMT6575ZSDPreview.u4Dirty= 0;
    spin_unlock_irqrestore(&g_stMT6575ZSDPreview.SpinLock , flag);

    if(copy_from_user(&g_stMT6575ZSDPreview.stReg , (struct stZSDPreviewReg *)a_u4Param , sizeof(stZSDPreviewReg)))
    {
    ...
    */
    ...
    {
        unsigned long y_cnt = (ioread32(MT6575_RZ_INFO0(CRZ_BASE))>>16) & 0x1FFF ;
        //unsigned long outcnt = ioread32(MT6575_RZ_INFO1(CRZ_BASE));
        unsigned long y_size = (ioread32(MT6575_RZ_SRCSZ_ADDR(CRZ_BASE))>>16) & 0xFFFF;

        if (y_cnt > y_size - 50)
        {
            spin_lock_irqsave(&g_stMT6575ZSDPreview.SpinLock , flag);
            g_stMT6575ZSDPreview.u4Dirty = 1;
            spin_unlock_irqrestore(&g_stMT6575ZSDPreview.SpinLock , flag);
            g_stMT6575ZSDPreview.stReg.u4DescUpdateZone = 0;
            MDPK_DEBUG("[ZSD] update zone unsafe y_cnt %d y_size %d, will update in CRZ SOF\n",
                       y_cnt, y_size);
        ...
        {
            UpdateZsdPreviewReg();
            g_stMT6575ZSDPreview.stReg.u4DescUpdateZone = 1;
            MDPK_DEBUG("[ZSD] update zone safe, update immediately y_cnt %d y_size %d\n",
                       y_cnt, y_size);
        }
    ...
    else
    {
        ...
        spin_unlock_irqrestore(&g_stMT6575ZSDPreview.SpinLock , flag);
        g_stMT6575ZSDPreview.stReg.u4DescUpdateZone = 0;
        MDPK_DEBUG("[ZSD] update zone unsafe, will update in CRZ SOF\n");
    }

    if(copy_to_user((struct stZSDPreviewReg *)a_u4Param, &g_stMT6575ZSDPreview.stReg,
...
{
    if(g_stMT6575ZSDPreview.u4Dirty)
    {
        MDPK_ZSDINFO("[ZSD ]Update reg in CRZ SOF\n");
        UpdateZsdPreviewReg();
        g_stMT6575ZSDPreview.u4Dirty = 0;
    }
}
                        ...
                        unsigned long   a_u4Param )
{
    ...
    switch(a_u4Command)
    {
        ...
        // last preview frame for zsd
        case MT6575MDP_T_SETZSDPREVIEWFRAME:
            i4RetVal = MT6575_SetZSDPreview((stZSDPreviewReg *)a_u4Param);
            break;
