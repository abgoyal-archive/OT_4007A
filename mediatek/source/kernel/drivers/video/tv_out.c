    ...
    do {                            \
        if (is_tv_enable_time_profiling) {\
            tv_time_end = _GetTime();   \
             if (tv_time_end-tv_time_start > 2) TV_WARN("Too Long Time: %u\n", tv_time_end-tv_time_start); \
        } \
    } while (0)
#else
#define TV_TIME_PROFILE_START()     \
...
inline UINT32 tvbuf_DumpStatus(void)
{
    int i;
    UINT32 status = 0;
...
inline void tvbuf_SetTimeStamp(int ID)
{
    UINT32 curTime;
...
inline UINT32 tvbuf_Reset(void)
{
    int i;
    //TVBUF_LOCK();
    for (i = 0; i < TVOUT_BUFFERS; i++)
...
inline UINT32 tvbuf_GetEmptyBuf(void)
{
    int i;
    ...
    for (i = 0; i < TVOUT_BUFFERS; i++)
    {
        if (tvBuf[i].buf_sts == TV_BUF_EMPTY)
        {
            ...
            tvbuf_SetTimeStamp(i);
            TV_LOG("TVR id:%d Sts: %x", tvBuf[i].buf_id, tvbuf_DumpStatus());
...
inline UINT32 tvbuf_GetFullBuf(void)
{
    int i;
    for (i = 0; i < TVOUT_BUFFERS; i++)
    {
...
inline TVOUT_STATUS tvbuf_SetBufReady(UINT32 buf_addr)
{
    int i;
...
inline UINT32 tvbuf_FindEmptyBuf(void)
{
    int i;
    for (i = 0; i < TVOUT_BUFFERS; i++)
    {
        if (tvBuf[i].buf_sts == TV_BUF_EMPTY)
        {
            ...
            tvbuf_SetTimeStamp(i);
            TV_LOG("TVC id:%d Sts: %x", tvBuf[i].buf_id, tvbuf_DumpStatus());
            return tvBuf[i].buf_id;
        }
    }
...
inline UINT32 tvbuf_FindFullBuf(void)
{
    int i;
    for (i=0; i<TVOUT_BUFFERS; i++)
    {
...
inline TVOUT_STATUS tvbuf_ReleaseBuf(UINT32 buf_addr)
{
    int i;
    if (buf_addr == 0)
    {
...
inline UINT32 tvbuf_GetBufID(UINT32 buf_addr)
{
    int i;
    if (buf_addr == 0)
    {
...
static __inline TVOUT_STATUS tvout_enterMirrorMode(void)
{
    ...
    TVBUF_LOCK();
    nextBuf = tvbuf_FindEmptyBuf();
    if (nextBuf == -1)
    {
        TV_WARN("no emtpy buf, try to find full buf");
        nextBuf = tvbuf_FindFullBuf();
        if (nextBuf == -1)
        {
...
static TVOUT_STATUS tvout_leaveMirrorMode(void)
{
    ...
    if (tvout_context.isEnabled)
    {
        ...
        TVBUF_LOCK();
        tvbuf_ReleaseBuf(TVC_GetWorkingAddr());
        TVBUF_UNLOCK();
    }
	return TVOUT_STATUS_OK;
}
...
static __inline TVOUT_STATUS tvout_enterVideoPlaybackMode(void)
{
	...
	if (_m4u_tvout_func.isInit && !is_m4u_tvc_port_closed && !tvout_context.isVideoM4uEnabled)
	{
		M4U_PORT_STRUCT M4uPort;
		...
		TV_INFO("Enter Video mode, turn off m4u tvc port");
		TVC_Enable();
	}
...
static __inline void tvout_leaveVideoPlaybackMode(void)
{
	...
	if (_m4u_tvout_func.isInit && is_m4u_tvc_port_closed && !tvout_context.isVideoM4uEnabled)
	{
        ...
        // disable TVC firstly, otherwise M4U will issue translate fault
        if (tvout_context.isEnabled == true)
        {
...
{
	TV_INFO("TvOut mode (%s)",(enable?"Enable":"Disable"));
...
#if defined(MTK_TVOUT_ENABLE_M4U)
            // Cable has been plug out, clear video buffer if SF did
            // not call leaveVideoBuffer
		...
		UINT32 width, UINT32 height)
{
	...
	if (pa == 0) {
		TV_ERROR("Can not get available pa!");
		return TVOUT_STATUS_ERROR;
	}
    temp = *(char *)va;
...
TVOUT_STATUS TVOUT_LeaveVideoBuffer(void)
{
	...
	if (tvout_context.isEnabled && tvout_context.tvMode == TVOUT_MODE_VIDEO )
    {
        ...
        // delete all tvout_video_buffer_list
#if defined(MTK_TVOUT_ENABLE_M4U)
        // enable TVC
        if (!tvout_context.isVideoM4uEnabled)
        {
            TVC_CHECK_RET(TVC_Enable());
        }
...
TVOUT_STATUS TVOUT_TvTurnOn(bool on)
{
    ...
    if (is_tv_out_turned_on && !is_tv_cable_plugged_in) {
        ...
        TV_INFO("Turn Off, Let Accdet detect cable");
...
static inline void _tvout_on_tvc_done(void)
{
    ...
    if (tvout_context.isEnabled && TVOUT_MODE_MIRROR == tvout_context.tvMode)
    {
        ...
        {
            TV_WARN("Tearing may occurs, get the same buffer %x", tvbuf_DumpStatus());
        }
        TVC_CHECK_RET(TVC_SetSrcRGBAddr(tvBuf[nextBuf].buf_mva_tvc));
#else
...
inline void  TVOUT_On_TVR_Done(void)
{
    ...
    if (tvout_context.isEnabled && tvout_context.tvMode == TVOUT_MODE_MIRROR)
    {
        ...
        tvbuf_SetBufReady(pre_inqueue_addr - tvr_buf_offset);
...
#else
static int TVout_Drv_Ioctl(struct file * a_pstFile,
								unsigned int a_Command,
								unsigned int a_Param)
#endif
{
    int ret = 0;
