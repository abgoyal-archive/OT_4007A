...
int ipanic_check_header(const struct ipanic_header *hdr) 
{
	...
	if ((hdr->oops_header_length < 16) || (hdr->oops_header_length > AEE_IPANIC_DATALENGTH_MAX)) {
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG, "aee-ipanic: No panic data available [Invalid oops header length - %d]\n", hdr->oops_header_length);
		return 2;
	}

	if ((hdr->oops_detail_length < 16) || (hdr->oops_detail_length > AEE_IPANIC_DATALENGTH_MAX)) {
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG, "aee-ipanic: No panic data available [Invalid oops detail length - %d]\n", hdr->oops_detail_length);
		return 2;
	}

	if ((hdr->console_length < 16) || (hdr->console_length > 128 * 1024)) {
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG, "aee-ipanic: No panic data available [Invalid oops console length - %d]\n", hdr->console_length);
		return 2;
	}
	return 0;
}
