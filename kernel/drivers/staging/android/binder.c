	...
	for (; offp < off_end; offp++) {
		...
		case BINDER_TYPE_FD: {
			...
			if (target_fd < 0) {
				fput(file);
#ifdef BINDER_MONITOR
				binder_user_error("binder: %d:%d no unused fd available, %ld\n",
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	while ((n = rb_first(&proc->allocated_buffers))) {
		...
		if (t) {
...
#ifdef BINDER_MONITOR
			printk(KERN_ERR
		   "%d: %p from %d:%d to %d:%d code %x flags %x pri %ld r%d ckpoint %u start %lu.%02lu",
		   t->debug_id, t,
		   ...
		   t->checkpoint,
		   (unsigned long)t->timestamp.tv_sec, (t->timestamp.tv_nsec / (NSEC_PER_SEC / 100)));
#endif
		}
		binder_free_buf(proc, buffer);
		buffers++;
...
#ifdef BINDER_MONITOR
	seq_printf(m,
		   "%d: %s from %d:%d to %d:%d node %d handle %d (%s) size %d:%d code %d start %lu.%02lu\n",
		   e->debug_id, (e->call_type == 2) ? "reply" :
		   ((e->call_type == 1) ? "async" : "call "), e->from_proc,
		   e->from_thread, e->to_proc, e->to_thread, e->to_node,
		   e->target_handle, e->service, e->data_size, e->offsets_size, e->code,
		   (unsigned long)e->timestamp.tv_sec, (e->timestamp.tv_nsec / (NSEC_PER_SEC / 100)));
#else
	seq_printf(m,
