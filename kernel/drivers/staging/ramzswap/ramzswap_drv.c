...
#define get_profile_time() sched_clock()
//#define get_profile_time() current->se.sum_exec_runtime

#undef kunmap_atomic
#define kunmap_atomic(addr, idx)	do { barrier(); dec_preempt_count();} while (0)

// added by KT for CompCache performance profiling
unsigned long long swap_in_time = 0;
unsigned long num_swap_in_pages = 0;
unsigned long long swap_out_time = 0;
unsigned long num_swap_out_pages = 0;
unsigned long long compress_time = 0;
unsigned long num_compress_pages = 0;
unsigned long long decompress_time = 0;
unsigned long num_decompress_pages = 0;
int mt_comp_cache_prof_enabled = 0;

unsigned long long uncompress_page_alloc_time = 0;
unsigned long long compress_page_alloc_time = 0;
unsigned long long memcpy_time = 0;
unsigned long long mutex_lock_time = 0;
unsigned long long mutex_lock_time_min = 0xFFFFFFFF;
unsigned long long mutex_lock_time_max = 0;
unsigned long long mutex_unlock_time = 0;
unsigned long long mutex_unlock_time_min = 0xFFFFFFFF;
unsigned long long mutex_unlock_time_max = 0;

unsigned long long handle_zero_page_time = 0;
unsigned long long handle_swap_fault_time = 0;
unsigned long long handle_uncompressed_page_time = 0;
unsigned long long kunmap_time = 0;
unsigned long long kunmap_time_min = 0xFFFFFFFF;
unsigned long long kunmap_time_max = 0;
unsigned long long flush_dcache_time = 0;

unsigned long long r1_time = 0, r2_time = 0, r3_time = 0, r4_time = 0;
...
{
	unsigned long long lat1=0,lat2=0;
	int flag = 0;

if (mt_comp_cache_prof_enabled)
	lat1 = get_profile_time();
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		flush_dcache_time += (lat2 - lat1);
}
...
static int handle_zero_page(struct bio *bio)
{
	...
	unsigned long long lat1,lat2,duration;	
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		duration = (lat2 - lat1);
	    kunmap_time += duration;
	    if(duration > kunmap_time_max)
	        kunmap_time_max = duration;
	    if(duration < kunmap_time_min)
	        kunmap_time_min = duration;
}	
...
static int ramzswap_read(struct ramzswap *rzs, struct bio *bio)
{
	...
	unsigned long long lat1,lat2,duration;
	unsigned long long lat3,lat4;
  
if (mt_comp_cache_prof_enabled)
	lat3 = get_profile_time();	 
...
if (mt_comp_cache_prof_enabled)
	lat1 = get_profile_time();	    
	    ...
	    ret = handle_zero_page(bio);
	    
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
	    handle_zero_page_time += (lat2 - lat1);
}	    
		return ret;
    }
...
if (mt_comp_cache_prof_enabled)
	lat1 = get_profile_time();	    
	    ...
	    ret = handle_ramzswap_fault(rzs, bio);

if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
	    handle_swap_fault_time += (lat2 - lat1);
}	   
...
if (mt_comp_cache_prof_enabled)
	lat1 = get_profile_time();
	    ...
	    ret = handle_uncompressed_page(rzs, bio);

if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
	    handle_uncompressed_page_time += (lat2 - lat1);
}	 	    
...
if (mt_comp_cache_prof_enabled) {
	...
	if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		decompress_time += (lat2 - lat1);
		num_decompress_pages++;
	}
...
unsigned short my_page_index[MAX_BUFFER_LENGTH];
void* my_page_buf;
int buf_length = 0;

void copy_to_memory_buffer(struct page* page, u32 index)
{
    int i;
    ...
    else if(i < buf_length)
    {
        user_mem = kmap_atomic(page, KM_USER0);
        dst = my_page_buf + i*4096;
        memcpy(dst, user_mem, 4096);
        kunmap_atomic(user_mem, KM_USER0);
    }
    else if(i == buf_length)
    {
        my_page_index[i] = index;
        user_mem = kmap_atomic(page, KM_USER0);
        dst = my_page_buf + i*4096;       
        memcpy(dst, user_mem, 4096);
        kunmap_atomic(user_mem, KM_USER0);        
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		mutex_time = (lat2 - lat1);
		mutex_lock_time += mutex_time;
		
		if(mutex_time > mutex_lock_time_max)
		    mutex_lock_time_max = mutex_time;
		if(mutex_time < mutex_lock_time_min)
		    mutex_lock_time_min = mutex_time;
}
...
copy_to_memory_buffer(page, index);
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		uncompress_page_alloc_time += (lat2 - lat1);
}
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		compress_page_alloc_time += (lat2 - lat1);
}
...
if (mt_comp_cache_prof_enabled) {
		lat2 = get_profile_time();
		mutex_time = (lat2 - lat1);
		mutex_unlock_time += mutex_time;
		
		if(mutex_time > mutex_unlock_time_max)
		    mutex_unlock_time_max = mutex_time;
		if(mutex_time < mutex_unlock_time_min)
		    mutex_unlock_time_min = mutex_time;		
}
...
static int __init ramzswap_init(void)
{
    ...
    my_page_buf = vmalloc(MAX_BUFFER_LENGTH*4096);
