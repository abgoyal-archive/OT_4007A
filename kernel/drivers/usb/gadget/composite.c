...
composite_unbind(struct usb_gadget *gadget)
{
	...
	switch_dev_unregister(&cdev->hw_connected);
...
composite_switch_work(struct work_struct *data)
{
	...
	if (is_hwconnected != cdev->hw_connected.state) {
		switch_set_state(&cdev->hw_connected, is_hwconnected);
		spin_lock_irqsave(&cdev->lock, flags);
		cdev->connected &= is_hwconnected;
		spin_unlock_irqrestore(&cdev->lock, flags);
	}
	...
	INFO(cdev, "composite_switch_work cnct=%x,cnct=%x,cfg=%x,hw=%x\n", connected, cdev->connected, \
                      (config==0x0) ? 0x0 : config->bConfigurationValue, is_hwconnected);
}
...
static int composite_bind(struct usb_gadget *gadget)
{
	...
	if (status < 0)
		goto fail;
	cdev->hw_connected.name = "usb_hwconnected";
	status = switch_dev_register(&cdev->hw_connected);
	if (status < 0)
		goto fail;
