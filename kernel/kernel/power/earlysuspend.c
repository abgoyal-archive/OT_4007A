...
static void early_suspend(struct work_struct *work)
{
	...
	list_for_each_entry(pos, &early_suspend_handlers, link) {
		if (pos->suspend != NULL)
		{    
        	if (lookup_symbol_name(pos->suspend, symname) < 0)
        		printk("Cannot mapping\n\r");
        	else
        		printk("[early_suspend]:name[%s],Level[%d],Addr[0x%x]\n\r", symname,pos->level,pos->suspend);

            //printk("[early_suspend]:pos->level= %d\n\r",pos->level);
            //printk("[early_suspend]:pos->suspend= 0x%x\n\r",pos->suspend);
	...
	list_for_each_entry_reverse(pos, &early_suspend_handlers, link)
    {
		...
		{    
        	if (lookup_symbol_name(pos->suspend, symname) < 0)
        		...
        		printk("[late_resume]:name[%s],Level[%d],Addr[0x%x]\n\r", symname,pos->level,pos->resume);
     	    
     	    //printk("[late_resume]:pos->level= %d\n\r",pos->level);
            //printk("[late_resume]:pos->resume= 0x%x\n\r",pos->resume);
