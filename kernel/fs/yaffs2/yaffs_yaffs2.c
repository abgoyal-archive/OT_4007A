...
#define YAFFS_MTK_RECHECK_PAGE_NUM 16

#define YAFFS_MTK_CHECK_BLOCKS 20
/*
...
int yaffs2_ScanBackwards(yaffs_Device *dev)
{
	...
	int check_block = YAFFS_MTK_CHECK_BLOCKS;
		...
		for (c = dev->param.nChunksPerBlock - 1;
			...
			if (!tags.chunkUsed) {
				...
				if(check_on&&(check_block>0)){					
					...
					retv = 0;
					for(i=0;i<YAFFS_MTK_RECHECK_PAGE_NUM;i++)
					{
					...
					if(retv<0){
						dev->nFreeChunks++;						
						printk(KERN_ERR"yaffsdebug re-check page fail!retv:%d i:%d/%d eccr:%d chunk:%d addr:0x%x cb:%d sn:%d\n",\
							retv,i,YAFFS_MTK_RECHECK_PAGE_NUM,tags.eccResult,chunk,chunk*dev->nDataBytesPerChunk,check_block,bi->sequenceNumber);
