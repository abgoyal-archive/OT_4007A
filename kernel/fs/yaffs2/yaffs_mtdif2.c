				       ...
				       __u8 *data, yaffs_ExtendedTags *tags)
{
	...
	if (dev->param.inbandTags || (data && !tags))
		...
		retval = mtd->read_oob(mtd, addr, &ops);
			if(retval)
			{
				printk(KERN_ERR"yaffsdebug mtdiferror %s retval:%d unfixed:%d fixed:%d chunk:%d addr:0x%x\n",\
					(retval == -EUCLEAN)?"fix ecc":"unfix ecc",retval,dev->eccUnfixed,dev->eccFixed,chunkInNAND,addr);
		...
		ops.mode = MTD_OOB_AUTO;
		ops.ooblen = packed_tags_size;
		ops.len = packed_tags_size;
		ops.ooboffs = 0;
		ops.datbuf = NULL;
		ops.oobbuf = yaffs_DeviceToLC(dev)->spareBuffer;
		oobret = mtd->read_oob(mtd, addr, &ops);
        ...
        {	
            ...
            ops.datbuf = data;
			ops.oobbuf = yaffs_DeviceToLC(dev)->spareBuffer;
            dataret = mtd->read_oob(mtd, addr, &ops);
			...
			if(retval)
			{
				...
				printk(KERN_ERR"yaffsdebug mtdiferror oobret:%d dataret:%d unfixed:%d fixed:%d chunk:%d addr:0x%x\n",oobret,dataret,dev->eccUnfixed,dev->eccFixed,chunkInNAND,addr);
				if(!dataret && oobret)
				{
					printk(KERN_ERR"dump dataok but ooberror checksum_BCH chunk:%d addr:0x%x\n",chunkInNAND,addr);
					mtk_dump_byte(ops.datbuf,ops.len,0);		
					printk(KERN_ERR"dump BCH oob\n");
					mtk_dump_byte(ops.oobbuf,ops.ooblen,0);
				}
			...
			T(YAFFS_TRACE_MTD, 
                    (TSTR ("nandmtd2_ReadChunkWithTagsFromNAND Re_Read retval=0x%x"TENDSTR), retval));
        }
		#endif
	}
#else
