...
{
	int ret = 1;
	if((((unsigned int)buffer)&0x3) || (nBytes&0x3))
	{
	
		__u32 *tmp = (__u32 *)buffer;
...
{ 
	...
	{
		printk(KERN_ERR"mtk_dump_word error p:0x%x count:%d\n",p,count);
		return;
	}
	...
	if (!init_failed) {
		...
		if (dev->param.isYaffs2) {
			if (yaffs2_CheckpointRestore(dev)) {
				...
				T(YAFFS_TRACE_ALWAYS,
				  (TSTR("yaffs: %s restored from checkpoint" TENDSTR),dev->param.name));
			} else {
