...
static HashSet<String>* supportedDownloadMIMETypes;
...
static void initializeSupportedDownloadMIMETypes()
{
    static const char* types[] = {
        "text/vnd.sun.j2me.app-descriptor"
    };
    for (size_t i = 0; i < sizeof(types)/sizeof(types[0]); ++i)
        supportedDownloadMIMETypes->add(types[i]);
}
...
static void initializeMIMETypeRegistry()
{
    ...
    supportedDownloadMIMETypes = new HashSet<String>;
    initializeSupportedDownloadMIMETypes();
}
...
bool MIMETypeRegistry::isSupportedDownloadMIMEType(const String& mimeType)
{
    if (mimeType.isEmpty())
        return false;
    if (!supportedDownloadMIMETypes)
        initializeMIMETypeRegistry();
    return supportedDownloadMIMETypes->contains(mimeType);
}
...
HashSet<String>& MIMETypeRegistry::getSupportedDownloadMIMETypes()
{
    if (!supportedDownloadMIMETypes)
        initializeSupportedDownloadMIMETypes();
    return *supportedDownloadMIMETypes;
}
