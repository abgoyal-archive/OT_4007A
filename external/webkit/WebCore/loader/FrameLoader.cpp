...
#else
static const char defaultAcceptHeader[] = "text/html,application/xml,application/xhtml+xml;q=0.9,text/plain;q=0.8,image/png,*/*;q=0.5";
#endif
...
#else    // USE(EXPAT)
#if ENABLE(WML)
        if (tokenizer->isHTMLTokenizer() == false) {
            // Workaround for xml declaration not at start of document.
            if (static_cast<XMLTokenizer *>(tokenizer)->isFirstChunk()) {
                static_cast<XMLTokenizer *>(tokenizer)->setFirstChunk(false);
                unsigned start = 0;
