...
void* IconDatabase::syncThreadMainLoop()
{
    ...
    while (!m_threadTerminationRequested) {
        ...
        // Ensure URL import is called after removeAllIcons, otherwise FrameLoader::startIconLoader() will never get a IconLoadYes unless icon URL already exists in DB and expired.
        // This is important due to CTS test case: WebChromeClientTest#testOnReceivedIcon
        if (kickoffURLImport) {
            kickoffURLImport = false;
            performURLImport();
        }
