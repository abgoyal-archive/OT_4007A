...
enum ThemeColorIndex { // keep this in sync with WebView.java
    SELECTION_CONTROLLER_FILL_STROKE_COLOR_INDEX = 0,
    SELECTION_CONTROLLER_FILL_TOP_COLOR_INDEX = 1,
    SELECTION_CONTROLLER_FILL_BOTTOM_COLOR_INDEX = 2,
    SELECTION_REGION_COLOR_INDEX = 3,
    CURSOR_OUTER_SELECTED_COLOR_INDEX = 4,
    CURSOR_OUTER_PRESSED_COLOR_INDEX = 5,
    CURSOR_INNER_SELECTED_COLOR_INDEX = 6,
    CURSOR_INNER_PRESSED_COLOR_INDEX = 7,
    CURSOR_PRESSED_FILL_COLOR_INDEX = 8
};
    ...
    m_ring((WebViewCore*) viewImpl)
{
 ...
 //   m_javaGlue = new JavaGlue;
    ...
    //Get color from current system theme.
    m_javaGlue.m_getThemeColor = GetJMethod(env, clazz, "getThemeColor", "(I)I");
    m_javaGlue.m_isUseTheme = GetJMethod(env, clazz, "isUseTheme", "()Z");
    ...
    bool isUseTheme = false;
    isUseTheme = env->CallBooleanMethod(javaWebView, m_javaGlue.m_isUseTheme);
    checkException(env);
    if (isUseTheme) {
        //Use system theme, init selection controller color, selection region color,
        //cursor ring color by current theme.
        setSelectionColorByTheme();
        setCursorRingColorByTheme();
    } else {
        //Don't use system theme.
        setSelectionColorByDefault();
        setCursorRingColorByDefault();
    }
}
...
void setSelectionColorByTheme(){
    JNIEnv* env = JSC::Bindings::getJNIEnv();
    int color[3];
    color[SelectText::STROKE_COLOR_INDEX] = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, SELECTION_CONTROLLER_FILL_STROKE_COLOR_INDEX);
    color[SelectText::FILL_GRADIENT_TOP_INDEX] = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, SELECTION_CONTROLLER_FILL_TOP_COLOR_INDEX);
    color[SelectText::FILL_GRADIENT_BOTTOM_INDEX] = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, SELECTION_CONTROLLER_FILL_BOTTOM_COLOR_INDEX);
    checkException(env);
    m_selectText.initSelectionController(true, color);
    int regionColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, SELECTION_REGION_COLOR_INDEX);
    checkException(env);
    m_selectText.setSelectionRegionColor(regionColor);
}

void setCursorRingColorByTheme(){
    JNIEnv* env = JSC::Bindings::getJNIEnv();
    int selectedColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, CURSOR_OUTER_SELECTED_COLOR_INDEX);
    int pressedColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, CURSOR_OUTER_PRESSED_COLOR_INDEX);
    checkException(env);
    m_ring.setCursorOuterColors(selectedColor, pressedColor);
    selectedColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, CURSOR_INNER_SELECTED_COLOR_INDEX);
    pressedColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, CURSOR_INNER_PRESSED_COLOR_INDEX);
    checkException(env);
    m_ring.setCursorInnerColors(selectedColor, pressedColor);
    pressedColor = env->CallIntMethod(m_javaGlue.object(env).get(),
            m_javaGlue.m_getThemeColor, CURSOR_PRESSED_FILL_COLOR_INDEX);
    checkException(env);
    m_ring.setCursorPressedColor(pressedColor);
}

void setSelectionColorByDefault(){
    //init selection controller use default color
    m_selectText.initSelectionController(false, NULL);
    //Default selection region color have been set in SelectText.cpp
}

void setCursorRingColorByDefault(){
    //Default color have been set in android_graphics.cpp
}
...
bool wordSelection(int x, int y)
{
    ...
    XLOGD2("WebView::wordSelection start at %d",startTime);
    startSelection(x, y);
    if (!extendSelection(x, y))
        return false;
    m_selectText.setDrawPointer(false);
    SkPicture* picture = getFrameCache(DontAllowNewer)->pictureAt(x, y);
    bool ret = m_selectText.wordSelection(picture);
    
    uint32_t endTime = getThreadMsec();
    XLOGD2("WebView::wordSelection end at %d, cost %d",endTime, endTime - startTime);
    return ret;
}
