    ...
    *frameNamePtr = '\0';
    ...
    keepTextNode:
		...
		if(nodeRenderer != NULL)
	    {
	        if (nodeRenderer->hasLayer()) 
			{
				RenderLayer* _track_layer = toRenderBoxModelObject(nodeRenderer)->layer();			
				RenderLayerBacking* _track_back = _track_layer->backing();
				if (_track_back != NULL)
				{
					GraphicsLayerAndroid* _track_grLayer = static_cast<GraphicsLayerAndroid*>(_track_back->graphicsLayer());
					if (_track_grLayer != NULL)
					{
						//Add the condition to check if it is a VALID Layer and the CacheNode bounds, if that we then add it into Layer.
                ...
                switch (patternChar) {
                    ...
                    case '0':
                        ...
                        if (ch < patternChar || ch > '9') {
                            ...
                            // Handle +0-000- case
                            // Handle +00-00- case 
                            // Handle 00-0000-0000 case
                            if ((pattern - s->mPattern == 10 ||
								 //Add One case to support +86-25-51887700
								 //Add One case to support +1-647-802-7708
								 //Add one case to support    02-2794-1111
								 pattern - s->mPattern == 5  ||
								 pattern - s->mPattern == 2  ||
								 pattern - s->mPattern == 9
								 //And End
								 ) && (ch == ' ' || ch == '-' || ch == ')'))
                                goto commonPunctuation;
                            goto resetPattern;
