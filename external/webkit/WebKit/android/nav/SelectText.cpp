...
#define XLOG_TAG "WebCore/SelectText"
...
void printSelected(const void* text, size_t byteLength){
   
   XLOGD2("printSelected");
   for(size_t i=0;i<byteLength;i++)
      XLOGD2("%x-",*((uint8_t *)text + i));
...
public:
    ...
    bool isComplexText() {
       if(mPaint!= NULL)
          return (mPaint->getTextEncoding() == SkPaint::kUTF16_TextEncoding);
       else
          return false;
    }
...
public:
    ...
    const Cluster& adjustedCluster(const SkIRect& area, int* base)
    {
        *base = mBestBase + area.fTop;
        mBestBounds.offset(area.fLeft, area.fTop);
        mBestCluster.startRect.offset(area.fLeft, area.fTop);
        mBestCluster.endRect.offset(area.fLeft, area.fTop);

        //XLOGD2("FirstCheck mBestClusterStarts:(%d, %d, %d, %d)",
        //mBestCluster.startRect.fLeft, mBestCluster.startRect.fTop, mBestCluster.startRect.fRight, 
        //mBestCluster.startRect.fBottom);

        //XLOGD2("FirstCheck mBestClusterEnds:(%d, %d, %d, %d)",
        //mBestCluster.endRect.fLeft, mBestCluster.endRect.fTop, mBestCluster.endRect.fRight, 
        //mBestCluster.endRect.fBottom);
        ...
        const SkBounder::GlyphRec& rec)
    {
        ...
        if (rec.clusterLen > 0) {
           preClusterStart.set(rect.fLeft, top(), rect.fRight, bottom());
           preClusterEnd.set(rect.fLeft, top(), rect.fRight, bottom());
           isStartRecordBest = false;
        }
        if (rec.clusterLen == 0) {
           preClusterEnd.set(rect.fLeft, top(), rect.fRight, bottom());
        }

        //if it's not complex text
        if (!isComplexText()) {
           isStartRecordBest = false;
        }
#endif
...
#ifdef MTK_SHAPE_ENGINE_SUPPORT
            mBestCluster.startRect = preClusterStart;
            //XLOGD2("bestGlyph: %d",rec.fGlyphID);

            if (!isComplexText()) {
              mBestCluster.startRect = mBestBounds;
              mBestCluster.endRect = mBestBounds;
            } else {
              isStartRecordBest = true;
            }          
#endif                      
            mDistance = distance;
            if (mRecordGlyph)
...
#ifdef MTK_SHAPE_ENGINE_SUPPORT
            if (isStartRecordBest) {
                mBestCluster.endRect = preClusterEnd;
                //XLOGD2("endGlyph: %d",rec.fGlyphID);
            }         
#endif 
...
protected:
    int mBestBase;
    SkIRect mBestBounds;
    struct Cluster mBestCluster;
    int mDistance;
    int mFocusX;
    int mFocusY;
    bool mRecordGlyph;
 
    bool isStartRecordBest;
    SkIRect preClusterStart;
    SkIRect preClusterEnd;
...
#ifndef MTK_SHAPE_ENGINE_SUPPORT
        if (mLastUni) {
            uint16_t chars[2];
...
#else
        // for complex text
        bool isComplexText = mPaint->getTextEncoding() == SkPaint::kUTF16_TextEncoding;
        if (mLastUni && !isComplexText) {
            uint16_t chars[2];
            size_t count = SkUTF16_FromUnichar(mLastUni, chars);
            *mSelectText.append() = chars[0];
            if (count == 2)
                *mSelectText.append() = chars[1];
            //XLOGD2("mLastUni:%d",count);
            printSelected(chars,count*2);
        }
        if (isComplexText && rec.clusterLen > 0 && rec.clusterLen <= MAX_CLUSTER_LEN){
            mLastUni = 0;

            // Reserve a cluster to visual order, so it will be reserve to right order in WebCore::String text().
            // Reverse as needed if result contains right-to-left characters.
            const uint16_t* start = rec.cluster;
            const uint16_t* end = rec.cluster + rec.clusterLen;
            while (start < end) {
              ...
              if (WTF::Unicode::RightToLeftArabic == charDirection
                    ...
                    WebCore::ReverseBidi((UChar *)rec.cluster, rec.clusterLen);
                    break;
              }
            }
            
            mSelectText.append(rec.clusterLen,rec.cluster);
            printSelected(rec.cluster,rec.clusterLen*sizeof(uint16_t));
        }
#endif  
...
static Cluster findClosestCluster(FirstCheck& _check, const SkPicture& picture,
        const SkIRect& area, int* base)
{
    ...
    checker.drawPicture(const_cast<SkPicture&>(picture));
    _check.finishGlyph();
    return _check.adjustedCluster(area, base);
}
        ...
        int x, int y, bool left, int* base)
{
    ...
    uint32_t loopStart = getThreadMsec();
    do { // detect left or right until there's a gap
        ...
        uint32_t drawStart = getThreadMsec();
        TextCanvas checker(&edge, area);
        checker.drawPicture(const_cast<SkPicture&>(picture));
        uint32_t drawEnd = getThreadMsec();
        if (drawEnd - drawStart > DRAW_TOO_LONG) {
            XLOGD2("FindEdge.drawPicture takes too long ");
            break;
        }
        ...
        *base = nextBase;
        ...
        uint32_t loopTimes = getThreadMsec();
        if (loopTimes - loopStart > LOOP_TOO_LONG) {
            XLOGD2("FindEdge.loop takes too long ");
            break;
        }
...
SelectText::SelectText()
{
    ...
    m_picture = 0;
    m_useTheme = false;
    m_SelectionRegionColor = 0;
}
void SelectText::initSelectionController(bool useTheme, int color[]){
    m_useTheme = useTheme;
    SkPaint paint;
    ...
    if (useTheme) {
        //convert int to SkCorlor, which is unsigned int.
        gradientColors[0]= (SkColor)color[FILL_GRADIENT_TOP_INDEX];
        gradientColors[1]= (SkColor)color[FILL_GRADIENT_BOTTOM_INDEX];
    } else {
        gradientColors[0]= FILL_GRADIENT_TOP;
        gradientColors[1] = FILL_GRADIENT_BOTTOM;
    }
    ...
    if (useTheme) {
        paint.setColor((SkColor)color[STROKE_COLOR_INDEX]);
    } else {
...
void SelectText::setSelectionRegionColor(int color){
    m_SelectionRegionColor = color;
}
...
void SelectText::drawSelectionRegion(SkCanvas* canvas)
{
    ...
    //make sure the m_selStart&m_selEnd included.
    m_selRegion.op(m_selStart,SkRegion::kUnion_Op);
    m_selRegion.op(m_selEnd,SkRegion::kUnion_Op);
    ...
    paint.setAntiAlias(true);
    if (m_useTheme) {
        paint.setColor((SkColor)m_SelectionRegionColor);
    } else {
...
#else
        mStartCluster = mEndCluster = findClosestCluster(center, *picture, clipRect, &base);
        m_selStart = mStartCluster.startRect;
        m_selEnd = mStartCluster.endRect;
#endif
...
#else
    Cluster found = findClosestCluster(extension, *picture, clipRect, &base);
    if (m_hitTopLeft) {
        m_startBase = base;
        m_selStart = found.startRect;
        mStartCluster = found;
        
    } else {
        m_endBase = base;
        m_selEnd = found.endRect;
        mEndCluster = found;
    }
#endif
    swapAsNeeded(); 
...
#else
    Cluster found = findClosestCluster(center, *picture, clipRect, &base);
    if (m_hitTopLeft || !m_extendSelection) {
        m_startBase = base;
        m_selStart = found.startRect;
        mStartCluster = found;
...
#else
        SkTSwap(mStartCluster, mEndCluster);
        m_selStart = mStartCluster.startRect;
        m_selEnd = mEndCluster.endRect;

        //XLOGD2("FirstCheck swapAsNeeded Start:(%d, %d, %d, %d)",
        //m_selStart.fLeft, m_selStart.fTop, m_selStart.fRight, 
        //m_selStart.fBottom);
        //XLOGD2("FirstCheck swapAsNeeded End:(%d, %d, %d, %d)",
        //m_selEnd.fLeft, m_selEnd.fTop, m_selEnd.fRight, 
        //m_selEnd.fBottom);
#endif
        m_hitTopLeft ^= true;
