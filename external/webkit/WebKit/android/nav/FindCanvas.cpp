...
GlyphSet::GlyphSet(const SkPaint& paint, const UChar* lower, const UChar* upper,
    ...
    mBufSize = byteLength;
    if (mBufSize > MAX_STORAGE_COUNT) {
        mLowerGlyphs = new uint16_t[2*mBufSize];
    } else {
    ...
    mUpperGlyphs = mLowerGlyphs + mBufSize;
    int count2 = clonePaint.textToGlyphs(lower, byteLength, mLowerGlyphs, mBufSize);
    //SkASSERT(mCount == count2);
    count2 = clonePaint.textToGlyphs(upper, byteLength, mUpperGlyphs, mBufSize);
    //SkASSERT(mCount == count2);
    mCount = count2;
    //XLOGD2("glyphSet size:%d",mCount);
}
...
GlyphSet::GlyphSet& GlyphSet::operator=(GlyphSet& src) {
    ...
    mCount = src.mCount;
    mBufSize = src.mBufSize;
    if (mBufSize > MAX_STORAGE_COUNT) {
        mLowerGlyphs = new uint16_t[2*mBufSize];
    } else {
        mLowerGlyphs = &mStorage[0];
    }
    memcpy(mLowerGlyphs, src.mLowerGlyphs, 2*mBufSize*sizeof(uint16_t));
    mUpperGlyphs = mLowerGlyphs + mBufSize;
    return *this;
}
...
void FindCanvas::findHelper(const void* text, size_t byteLength,
    ...
    if (mWorkingIndex) {
        ...
        if (workingBounds.fTop > newYInt) {
            ...
            // for English and the like(non-CJK), a space must be provided to match two words.
            // for Chinese and the like(CJK), a space is not necessary to match two characters
            int nextBreakable = -1;
            bool breakable = isBreakable(mLowerText, mWorkingIndex, mLength / sizeof(UChar), nextBreakable, false);
            if (breakable) {
