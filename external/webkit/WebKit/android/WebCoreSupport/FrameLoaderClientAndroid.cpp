...
{
    LOGD("FrameLoaderClientAndroid::~FrameLoaderClientAndroid() this=0x%x", this);
...
void FrameLoaderClientAndroid::frameLoaderDestroyed() {
    LOGD("FrameLoaderClientAndroid::frameLoaderDestroyed() this=0x%x", this);
    //registerForIconNotification(false);
    //m_frame = 0;
    //Release(m_webFrame);
    delete this;
...
bool FrameLoaderClientAndroid::canShowMIMEType(const String& mimeType) const {
    // Can special mime types be download.
    if (MIMETypeRegistry::isSupportedDownloadMIMEType(mimeType))
        return false;
...
void FrameLoaderClientAndroid::registerForIconNotification(bool listen) {
    LOGD("FrameLoaderClientAndroid::registerForIconNotification(%d) this=0x%x", listen, this);
    if (listen)
