...
void WebViewCore::recordPictureSet(PictureSet* content)
{
    ...
    // avoid calling reuseSubdivided becuase of disabling splitting picture set 
...
void WebViewCore::scrollBy(int dx, int dy, bool animate)
{
    ...
    env->CallVoidMethod(javaObject.get(), m_javaGlue->m_scrollBy,
        dx, dy, animate);
    checkException(env);
}
...
void WebViewCore::restoreScale(int scale)
{
    ...
    env->CallVoidMethod(javaObject.get(), m_javaGlue->m_restoreScale, scale);
    checkException(env);
}
...
#if ENABLE(WML)
WebCore::WMLAnchorElement* WebViewCore::retrieveWMLAnchorElement(WebCore::Frame* frame, WebCore::Node* node)
{
    if (!CacheBuilder::validNode(m_mainFrame, frame, node))
        return 0;
...
WebCore::String WebViewCore::retrieveHref(WebCore::Frame* frame, WebCore::Node* node)
{
    ...
    if (node->isWMLElement()) {
        WebCore::WMLAnchorElement* anchor = retrieveWMLAnchorElement(frame, node);
...
void WebViewCore::openFileChooser(PassRefPtr<WebCore::FileChooser> chooser) {
    ...
    jstring jName = (jstring) env->CallObjectMethod(
            javaObject.get(), m_javaGlue->m_openFileChooser);
    checkException(env);
...
bool WebViewCore::jsPrompt(const WebCore::String& url, const WebCore::String& text, const WebCore::String& defaultValue, WebCore::String& result)
{
    ...
    jstring jUrlStr = env->NewString((unsigned short *)url.characters(), url.length());
    jstring returnVal = (jstring) env->CallObjectMethod(javaObject.get(), m_javaGlue->m_jsPrompt, jUrlStr, jInputStr, jDefaultStr);
...
void WebViewCore::setScrollbarModes(ScrollbarMode horizontalMode, ScrollbarMode verticalMode)
{
    ...
    env->CallVoidMethod(javaObject.get(), m_javaGlue->m_setScrollbarModes,
