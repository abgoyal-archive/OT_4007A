...
typedef struct _BWCS_MESSAGE_STRUCT {
    unsigned short usType; /*u16*/
    unsigned short bwcs_event; /*u16*/
    struct hci_conn_info  bt_conn;
    unsigned short conn_num;
    unsigned short data_len; /*u16*/
					   ...
					   void *ctx, char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case IWEVCUSTOM:
			...
			msg.usType = 0x01;
			msg.bwcs_event = BWCS_WIFI_SCM_UCM;
			msg.data_len = iwe->u.data.length;
			os_memcpy(msg.data, custom, iwe->u.data.length);

			if(bwcs_send_internal_event_msg(BWCS_WIFI_SCM_UCM, msg) < 0)
			{
				wpa_printf(MSG_ERROR, "bwcs wpa_supplicant discon socket error!");
			}
		#endif
