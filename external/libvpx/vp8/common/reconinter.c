...
{
    LOGV("vp8_build_inter_predictors_mb in");
        ...
        x->mode_info_context->mbmi.mode != SPLITMV)
    {
        ...
        pre_stride >>= 1;
        offset = (mv_row >> 3) * pre_stride + (mv_col >> 3);
        LOGV("x-pre.u_buffer -> %d, x->pre.y_buffer -> %d", x->pre.u_buffer, x->pre.u_buffer);
        uptr = x->pre.u_buffer + offset;
        vptr = x->pre.v_buffer + offset;
    ...
    LOGV("vp8_build_inter_predictors_mb out");
}
