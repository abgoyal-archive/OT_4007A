...
static int get_free_fb (VP8_COMMON *cm)
{
    ...
    if (i == NUM_YV12_BUFFERS) { // no free frame buffer available, update last one.
        LOGE("no free buffer available, update the last one");
        i = cm->new_fb_idx = cm->lst_fb_idx;        
    }
...
int vp8dx_receive_compressed_data(VP8D_PTR ptr, unsigned long size, const unsigned char *source, INT64 time_stamp)
{
    ...
    LOGV("decode %d frame, time : %d ms",cm->current_video_frame, pbi->decode_microseconds);
